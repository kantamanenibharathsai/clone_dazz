import { Box, CircularProgress, Grid, Typography } from "@mui/material";
import { colors, hex2rgba } from "../../../config/theme";
import DashBoardIndicator from "../../common/dashboardIndicator/DashBoardIndicator";

import {
  assignedScreens,
  expectedIncome,
  myWallet,
  offlineScreens,
  onlineScreens,
} from "../assets";
import { dashboardStyles } from "./dashBoardStyles";
import MyScreenLocations from "../../common/myScreenLocations/MyScreenLocations";
import { useDispatch, useSelector } from "react-redux";
import { AppDispatch, RootState } from "../../../redux/store";
import { FunctionComponent, useEffect } from "react";
import { getInvestorDashboardData } from "../../../redux/reducers/investorReducers/dashboardSlice";

const Dashboard = () => {
  const { data, loading, error } = useSelector(
    (state: RootState) => state.InvestorDashboardSlice
  );
  const dispatch = useDispatch<AppDispatch>();
  useEffect(() => {
    dispatch(getInvestorDashboardData());
  }, [dispatch]);
  const getDataFromAPI = (title: string) => {
    const getValue = (value?: number | undefined, change?: string | undefined, icon?: string) => {
      return {
        value: typeof value === 'string' ? parseFloat(value) || 0 : value ?? 0,
        percentage: change ? calculatePercentage(change) : undefined,
        icon: icon ? icon.slice(icon.indexOf("(") + 1, icon.indexOf(")")) : undefined,
      };
    };
    switch (title) {
      case "Online Screens":
        return getValue(data?.onlineScreens, data?.onlineScreenChange, data?.onlineScreenChange);
      case "Offline Screens":
        return getValue(data?.offlineScreens, data?.offlineScreenChange, data?.offlineScreenChange);
      case "Assigned Screens":
        return getValue(data?.totalScreens);
      case "Expected Income":
        return getValue(data?.expectedIncomeTotal, data?.expectedIncomeChange, data?.expectedIncomeChange);
      case "My Wallet":
        return getValue(data?.balanceAmount);
      case "Commission":
        return getValue(data?.commissionData);
      case "Invested Amount":
        return getValue(data?.investedAmountTotal, data?.investedAmountChange, data?.investedAmountChange);
      case "Linked Property":
        return getValue(data?.totalScreens);
      default:
        return { value: undefined, percentage: undefined, icon:undefined };
    }
  };
  const calculatePercentage = (change: string | undefined): number | undefined => {
    const parsedChange = parseFloat(change ?? "");
    return isNaN(parsedChange) ? undefined : parsedChange;
  };
  const screensData = [
    {
      id: 1,
      image: onlineScreens,
      title: "Online Screens",
      value: 145,
      percentage: 15,
      percentageLabel: "this month",
    },
    {
      id: 2,
      image: offlineScreens,
      title: "Offline Screens",
      value: 145,
      percentage: 15,
      percentageLabel: "this month",
    },
    {
      id: 3,
      image: assignedScreens,
      title: "Assigned Screens",
      value: 145,
      percentage: 15,
      percentageLabel: "this month",
    },
    {
      id: 4,
      image: expectedIncome,
      title: "Expected Income",
      value: 145,
      percentage: 15,
      percentageLabel: "this month",
    },
    {
      id: 5,
      image: myWallet,
      title: "My Wallet",
      value: 145,
      percentage: 15,
      percentageLabel: "this month",
    },
    {
      id: 6,
      image: expectedIncome,
      title: "Commission",
      value: 145,
      percentage: 15,
      percentageLabel: "this month",
    },
    {
      id: 7,
      image: expectedIncome,
      title: "Invested Amount",
      value: 145,
      percentage: 15,
      percentageLabel: "this month",
    },
    {
      id: 8,
      image: myWallet,
      title: "Linked Property",
      value: 145,
      percentage: 15,
      percentageLabel: "this month",
    },
  ];
  return (
    <Box mt={6}>
      <Grid container sx={dashboardStyles.mainContainer} rowSpacing={5}>
      {loading && (
          <Grid
            item
            xs={12}
            display={"flex"}
            justifyContent={"center"}
            alignItems={"center"}
            padding={10}
          >
            <CircularProgress />
          </Grid>
        )}
        {error && (
          <Grid
            item
            xs={12}
            display={"flex"}
            justifyContent={"center"}
            alignItems={"center"}
            padding={10}
          >
            <Typography variant="h6" color="error">
              {error.toString()}
            </Typography>
          </Grid>
        )}
        {!loading &&
          !error && screensData.map((eachData) => (
          <Grid
            item
            key={eachData.id}
            xs={12}
            sm={6} 
            md={6}
            lg={4}
            xl={4}
            padding={{ xs: 3.5, md: 1.5, lg: 2, xl: 3.5 }}
          >
            <DashBoardIndicator
              title={eachData.title}
              value={getDataFromAPI(eachData.title).value}
              image={eachData.image}
              percentage={getDataFromAPI(eachData.title).percentage}
              icon={getDataFromAPI(eachData.title).icon}
              styles={{
                borderRight: {
                  xs: "none",
                  sm: `1px solid ${
                    eachData.id === screensData.length ||
                    eachData.id === screensData.length - 2 ||
                    eachData.id === screensData.length - 4 ||
                    eachData.id === screensData.length - 6
                      ? "transparent"
                      : hex2rgba(colors.black, 0.1)
                  }`,
                  lg: `1px solid ${
                    eachData.id === screensData.length ||
                    eachData.id === screensData.length - 2 ||
                    eachData.id === screensData.length - 5
                      ? "transparent"
                      : hex2rgba(colors.black, 0.1)
                  }`,
                  xl: `1px solid ${
                    eachData.id === screensData.length - 2 ||
                    eachData.id === screensData.length - 5 ||
                    eachData.id === screensData.length
                      ? "transparent"
                      : hex2rgba(colors.black, 0.1)
                  }`,
                },
              }}
            />
          </Grid>
        ))}
      </Grid>
      <Box sx={dashboardStyles.mapContainer}>
        <Typography variant="h5">My Screen Locations</Typography>
        <MyScreenLocations/>
      </Box>
    </Box>
  );
};

export default Dashboard as FunctionComponent;
