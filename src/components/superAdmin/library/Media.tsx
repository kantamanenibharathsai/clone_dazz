...
import { handleCardLayoutPlaceHolder, tokenWithUrl } from "../../../utils/utils";
    ...
    <Stack>
      ...
      {selectedMedia && selectedMedia!.id ? (
        ...
        <Stack direction={"row"} gap={2} my={"30px"} flexWrap={"wrap"}>
          {mediaMessage ? (
            <Typography sx={libraryStyles.noDataText}>{message}</Typography>
          ) : (
            mediaData?.length > 0 &&
            mediaData.map((each) => (
              <Stack
