...
import { formateDate, tokenWithUrl } from "../../../utils/utils";
...
const Playlist = () => {
  ...
  const handlePagination = (newPage: number) => {
    ...
    dispatch(getPlaylistData({ searchInput: "", page: newPage,sort:selectInput }));
  };
  ...
  useEffect(() => {
    if (searchInput.length === 0) {
      dispatch(getPlaylistData({ searchInput, page: currentPage,sort:selectInput }));
    } else {
                ...
                playlistData.map((layout, index) => {
                  ...
                  <Grid item xs={12} sm={6} md={12} lg={6} xl={4} key={index}>
                    <CardLayout
                      ...
                      handleEdit={
                        permission.edit ? () => handleEdit(index) : undefined
                      }
                      handleShow={() => handleShow(index)}
                      handleSettings={() => handleSettings(layout?.id)}
                      handleDelete={
                        permission.delete
                          ? () => handleDelete(index)
                          : undefined
