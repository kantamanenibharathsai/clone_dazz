...
import { volumeIcon } from "../../assets/Index";
...
const Configuration = () => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (allApiStatus.updateScreenConfiguration === "PENDING") return;
    let validate = "";
  ...
  const fillingFields = () => {
    ...
    setScreenOrientation(
      screenData.orientation?.toLocaleUpperCase() as ScreenOrientation
    );
    ...
    <Stack sx={configurationStyles.mainContainer}>
      <Box component={"form"} onSubmit={handleSubmit}>
        <Grid container spacing={2}>
            ...
            <Stack direction={"column"} sx={configurationStyles.formCon}>
              ...
              <Stack sx={configurationStyles.generalSection}>
                ...
                <Stack sx={configurationStyles.switchContainer}>
                  <Grid container>
                    {switchData.slice(0, 2).map((item, index) => (
                      <Grid
                        item
                        xs={6}
          ...
          <Grid item xs={12}>
            ...
            >
              ...
              <Stack direction={"row"} gap={3}>
                {switchData.slice(2, switchData.length).map((item, index) => (
                  <Box key={index}>
                    <FormControlLabel
          ...
          {permission.edit && (
            <CustomButton
              sx={configurationStyles.saveBtn}
              type="submit"
              disabled={
                allApiStatus.updateScreenConfiguration === "PENDING"
                  ? true
                  : false
              }
            >
